<template>
  <div class="bg-primary text-white m-2 p-2">
    ...
    <div class="text-white center my-2">
      <button class="btn btn-light" v-on:click="doChange">Zmień</button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  beforeUpdate() {
    console.log(
      `Wywołano metodę beforeUpdate. Stan zaznaczenia: ${this.checked}` +
  ...
  updated() {
    console.log(
      `Wywołano metodę updated. Stan zaznaczenia: ${this.checked}` +
        ` Imię: ${this.names[0]} Elementy listy: ` +
        this.$el.getElementsByTagName("li").length
    );
  },
